<template>
  <div class="addaddress">
    ...
    <van-nav-bar @click-left="onClickLeft">
      <template #title>
        <h1 class="text-[#333] text-[20px] font-bold"> 请填写发货地址 </h1>
      </template>
      <template #left>
        <van-icon name="arrow-left" size="28" color="#333" class="font-bold" />
      </template>
    </van-nav-bar>
    <!-- 中间部分，信息输入 -->
    <van-form class="middle-form" label-width="110">
      <van-cell-group>
        <!-- 地址选择 -->
        <van-field
          v-model="result"
        ...
        <van-field v-model="houseNumber" label="门牌号" name="mph" placeholder="请填写门牌号（非必填）" />
        <van-field
          v-model="userName"
          label="联系人"
          name="userName"
      ...
      <div class="smart-text-group">
        <div class="py-3 pl-[20px]"> 智能填写</div>
        <div>
          <van-field
            class="smart-text"
            maxlength="200"
          ...
          <van-button class="smart-btn" color="#303747" disabled round> 提交</van-button>
        </div>
      </div>
    </van-form>

    <div class="submit-panel">
      <van-button color="#303747" block @click="getDate()"> 确定</van-button>
    </div>
  </div>
</template>
  ...
  setup() {
    ...
    const getDate = function () {
      if (checkName() && checkPhone()) {
        $router.push(
          `/zj/writeaddress?houseNumber=${houseNumber.value}&userName=${userName.value}&phone=${phone.value}`
        )
      } else {
        return
      }
